...
The Schönflies Theorem generalizes the Jorden Curve Theorem. 
...
> [!Theorem] Schönflies
> Any $2$-sphere in $\mathbb{R}^{3}$ that is a submanifold bounds a $3$-ball. 

Note that the $2$-sphere must be a submanifold - there exists imbeddings of $\mathbb{S}^{2}$ into $\mathbb{R}^{3}$ (The Alexander horned sphere, for instance) which are not submanifolds. 
...
$\mathbb{S}^{3}$ is boundary irreducible, since it has no boundary. 
