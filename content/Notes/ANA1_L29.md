...
Similar definition exists for the Stieltjes integral. Note that parts [[ANA1_L27#a|a]], [[ANA1_L27#c|c]] and [[ANA1_L27#e|e]] of the properties of integrals are valid for these integrals, and so is the [[ANA1_L28#Integration and differentiation|fundamental theorem of calculus]]:
...
> **Proof**
> $|\mathbf{f}|=\sqrt{ f_{1}^{2}+f_{2}^{2}+\dots f_{k}^{2} }$. By definition, each $f_{i}\in\mathscr{R}$. It [[ANA1_L27#Compositions of integrable functions with continuous functions|follows]] that $f_{i}^{2}$ is integrable for each $i$, since $x^{2}$ is continuous (alternatively, the [[ANA1_L28#Product of integrable functions is integrable|product of two integrable functions is integrable]]). Also, note that $x^{2}$ is bijective and continuous on $[0, \infty)$. Thus, $\sqrt{ x }$ is continuous on $[0, \infty)$. It [[ANA1_L21#Continuous bijective maps on compact sets are homeomorphisms|follows]] that $|\mathbf{f}|\in\mathscr{R}$.
> 
...
> [!Definition]
> A [[ANA1_L11#Definition|continuous]] mapping $\boldsymbol{\gamma}:[a, b]\to \mathbb{R}^{k}$ is called a *curve*. If $\boldsymbol{\gamma}$ is injective, it is called an *arc*. If $\boldsymbol{\gamma}(b)=\boldsymbol{\gamma}(a)$, $\boldsymbol{\gamma}$ is called a *closed curve*. 

Consider a [[ANA1_L26#Partitions|partition]] $P=\{ x_{0}, x_{1}, \dots, x_{n} \}$ of $[a, b]$. Define
$$
\begin{align}
...
One way to ensure integrability is to require $\boldsymbol{\gamma}'$ to be continuous, [[ANA1_L15#Vector valued functions whose components are continuous|since that makes]] $|\boldsymbol{\gamma}'|$ continuous, and [[ANA1_L27#Continuous functions|continuous real valued functions are integrable]].
...
> **Proof**
> Fix $P$. From the [[ANA1_L28#Integration and differentiation|fundamental theorem of calculus]] and [[#Analogue of 6.13b|6.25]], we have
> $$
