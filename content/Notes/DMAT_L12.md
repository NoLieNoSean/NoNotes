...
> - $x\in \bigcup A_{i}$ $\implies$ $x$ contributes 1
> - $x\not\in \bigcup A_{i}$ $\implies$ $x$ contributes 0.
> 
> 
> Let $J$ be the largest index set such that that $x\in A_{J}$. Let $|J|=j$. Then, contribution of $x$ to the RHS is equal to
> $$
> \sum_{I\subseteq J}(-1)^{|I|}=\sum_{i=1}^{j} (-1)^{i}\binom{j}{i}=\begin{cases}
> (1-1)^{j}=0 & j> 0 ~\left(\iff x\not\in \bigcup A_{i}\right)\\
> 1 & j=0 ~\left(\iff x\in \bigcup A_{i}\right)
> \end{cases}
> $$
...
Let $X=[n]$. 
Consider all functions $f:P([n])\to \mathbb{R}$. These form a $2^{n}$ dimensional vector space over $\mathbb{R}$. 
