...
$$
is a basis for $V^{*\otimes k}$, which therefore has dimension $n^{k}$. 
...
$$
Its image is precisely the space of alternating tensors. Moreover, the quotient 
$$
...
> [!Important]
> As graded vector spaces, there is a canonical embedding
> $$
> \bigoplus_{m}\Lambda^{m}V^{*}\hookrightarrow\bigoplus_{m}V^{*\otimes m}=T(V).
> $$
...
The product formula is the neat
$$
\begin{align}  
...
\end{align}
$$
since $\text{Alt}(\varphi_{i_{1}}\otimes \dots \otimes \varphi_{i_{k}})  =\text{Alt}(\text{Alt}(\varphi_{i_{1}}\otimes \dots \otimes \varphi_{i_{p}})\otimes \text{Alt}(\varphi_{i_{p+1}}\otimes \dots \otimes \varphi_{i_{k}}))$ (this is easily verified by computing). 

The price you pay for using the identification [[#^fab235]] is that the familiar coordinate formulas acquire factorials. For example,
$$
dx^i\wedge dx^j =\frac12\left(dx^i\otimes dx^j-dx^j\otimes dx^i\right)  

$$
as alternating tensors, so  
$$
(dx^i\wedge dx^j)(e_i,e_j)=\frac12,  
$$
rather than $1$. Most differential geometers dislike this, because it makes every determinant formula come with compensating factorials. So they rescale the identification in each degree by $k!$, obtaining the convention used by @spivakCalculusManifoldsModern1965:
$$
i_{k}(\varphi_{i_{1}}\wedge\dots \wedge\varphi_{i_{k}})=k!\text{Alt}(\varphi_{i_{1}}\otimes \dots \otimes \varphi_{i_{k}}).
$$
The product formula becomes the slightly less pleasant
$$
\begin{align}  
...
\end{align}
$$
We will identify $\omega\in\Lambda^{k}V^{*}$ with its image $i_{k}(\omega)\in V^{*\otimes k}$ from now on. For instance, the above formula will be written as
$$
