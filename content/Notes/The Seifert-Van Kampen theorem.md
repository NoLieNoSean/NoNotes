...
[[Covering Spaces and the Fundamental Group#^3d339c|induce]] fundamental group homomorphisms
...
> 
> Then the homomorphism $\Phi:\pi_{1}(U, p)*\pi_{1}(V, p)\to \pi_{1}(X, p)$ induced by the universal property of coproducts descends to an isomorphism from the [[#^a3e029|amalgamated free product]] $\pi_{1}(U, p)*_{\pi_{1}(U\cap V, p)}\pi_{1}(V, p)$ to $\pi_{1}(X, p)$. 
...
The other special case is that in which one of the open subsets, say $U$, is simply connected. In this case, the diagram 
