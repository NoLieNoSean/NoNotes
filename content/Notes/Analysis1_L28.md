...
> [!Recall]
> [[Analysis1_L27#Which functions are integrable?|Integrability]] of bounded $f$ on $[a, b]$:
> 
...
> **Proof**
> Let $\epsilon>0$. Since $f\in\mathscr{R}$ on $[a, b]$, we can choose a partition $P$ of $[a, b]$ such that $U(P, f)-L(P, f)<\epsilon$. Because of the [[Analysis1_L24#Mean value theorem|mean value theorem]], we can choose $t_{i}\in[x_{i-1}, x_{i}]$ such that
> $$
...
> [!Theorem]
> If $f\in\mathscr{R}$ and $g\in\mathscr{R}$ on $[a, b]$, then $fg\in\mathscr{R}$.

> **Proof**
> We know that $f+g$ and $f-g$ are integrable on $[a, b]$, from [[Analysis1_L27#a|6.12a]]. We can [[Analysis1_L27#Compositions of integrable functions with continuous functions|compose]] these with $x^{2}$ to obtain integrable functions $(f+g)^{2}$ and $(f-g)^{2}$. It follows that $(f+g)^{2}+(f-g)^{2}=4fg$ is integrable. ❏
...
> **Proof**
> Let $H(x)=F(x)G(x)$. Then, $h\equiv H'=fG+Fg$. Note that since $f, g\in\mathscr{R}$ and $F, G\in\mathscr{R}$(since [[Analysis1_L23#Differentiability implies continuity|differentiability implies continuity]] and [[Analysis1_L27#Continuous functions|continuous functions are integrable]]), $h\in\mathscr{R}$. From the fundamental theorem, we have
> $$
