...
Let $C=\bigcup_{i\in I}C_{i}$ and $\mathbf{D}=\bigcup_{i\in I}\mathbf{D}_{i}$. It is easy to see that $\mathbf{D}$ is a  $\alpha$-covering for $C$. $(C, \mathbf{D})\in P$, so [[DMAT_L3#Zorn's lemma|Zorn's lemma]] is applicable. Let $(\hat{C}, \mathbf{\hat{D}})$ be the maximal element. 
