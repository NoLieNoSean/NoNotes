...
We will now generalize the notion of the inner product. We will see that the [[ALG1_L21#Inner Product Spaces|inner product, as defined in the previous lecture]], is a positive definite symmetric (bilinear) form on a real vector space, and a positive definite Hermitian form on a complex vector space. 
...
Let $\langle \,, \rangle$ be a bilinear form on a real vector space $V$, and let $A$ and $A'$ be its matrices with respect to bases $\mathcal {B}$ and $\mathcal {B'}$. Let $P$ be the the [[ALG1_L13#Change of basis|change of basis matrix]] from $\mathcal {B}$ to $\mathcal {B'}$ (i.e, $[\mathbf{v}]_{\mathcal {B}}=P[\mathbf{v}]_{\mathcal {B'}}$ for all $\mathbf{v}\in V$). Then, 
