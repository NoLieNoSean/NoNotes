...
For two vector spaces $V$ and $W$ with dimensions $p$ and $q$ and $T:V\to W$, $\mathcal{M}_{B_{V}, B_{W}}(T)$ is a $q \times p$ matrix, as we have [[ALG1_L12#Matrices of linear maps in fdvsp|seen]]. As every $T$ has a single matrix for a fixed set of bases,  $\mathcal{M}_{B_{V}, B_{W}}$ is a function. That is,
$$
\mathcal{M}_{B_{V}, B_{W}}:\hom(V, W)\to \{ q\times p \text{ matrices} \}.
$$
**Exercise:** Prove that $\mathcal{M}_{B_{V}, B_{W}}$ is an isomorphism of vector spaces. [[ALG1_L20#Recall|Solution here]]
...
Let $\text{rank} \ T = r$. Now, take a basis of $\ker T$, $\{\mathbf{k}_{1}, \dots \mathbf{k}_{p-r}\}$ (Recall the [[ALG1_L11#Rank nullity theorem for general linear maps over fdvsps|rank nullity theorem]]).
Extend to get a basis of $V$,  $B_{V} = \{ \mathbf{k}_{1}, \dots \mathbf{k}_{p-r}, \mathbf{v}_{1}, \dots \mathbf{v}_{r} \}$.
We have shown that $T(\mathbf{v}_{1}) \dots T(\mathbf{v}_{r})$ are a basis of $\text{Im} \ T$ (See [[ALG1_L11#Rank nullity theorem for general linear maps over fdvsps|here]]).
