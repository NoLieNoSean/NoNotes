...
>[!important] Remark
> If $V$ is a finite dimensional vector space and $\exists \ \mathbf{v} \in V$ such that $\mathbf{v} \neq 0$ then there is an $f \in \hat{V}$ such that $f(\mathbf{v}) \neq 0$. This is also true for infinite dimensional vector spaces.

## The double dual

> [!Definition] Double dual
> We will denote the dual space of the dual space of $V$ - the **double dual** of $V$ - by $\hat{\hat{V}} \equiv \hom_{\mathbb{F}}(\hat{V}, \mathbb{F})$.

For $\mathbf{v}_{0} \in V$, define corresponding $T_{\mathbf{v}_{0}} \in \hat{\hat{V}}$ ($T_{\mathbf{v}_{0}}:\hat{V}\to \mathbb{F}$) such that $f \xmapsto{T_{\mathbf{v}_{0}}} f(\mathbf{v}_{0})$. Define $\psi: V \to \hat{\hat{V}}$ by $\mathbf{v} \xmapsto{\psi} T_{\mathbf{v}}$.

Observe that $T_{\mathbf{v}}$ is linear over $\mathbf{v}$:
$$
...
\end{array}
$$
Now, from the [[LEC ALG1 11#Rank nullity theorem for general linear maps over fdvsps|rank nullity theorem]], $\dim \mathrm{Im}\ \psi=\dim V$. Note that $\dim V= \dim\hat{V}=\dim \hat{\hat{V}}$. Hence, $\psi$ is surjective. Therefore, $\psi$ is a bijective linear map, i.e, an isomorphism.
$$
\psi: V \overset{\sim}\longrightarrow \hat{\hat{V}}
$$
Notice that the definition of $\psi$ does not require a choice of basis. Such an isomorphism is called a *canonical isomorphism*. Contrast with an isomorphism $\phi:V\to \hat{V}$, which can be defined only after picking a basis. Thus, $V\cong \hat{V}$ is not canonical, while $V\cong \hat{\hat{V}}$ is.

We therefore have canonical isomorphisms
$$
