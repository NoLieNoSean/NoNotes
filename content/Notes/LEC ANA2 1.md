...
> $$
> Let $\mathscr{l}_{\infty}$ denote $(B(\mathbb{N}), \lVert \cdot \rVert_{\infty})$ (this is the space defined [[LEC ANA1 33#Another perspective on uniform convergence|here]] with $X=\mathbb{N}$, which we have shown to be an NLS). @kumaresanTopologyMetricSpaces2005 1.1.38 proves that $\mathscr{l}_{p}$ for $1\leq p< \infty$ is a NLS with the norm defined by $\lVert (a_{n}) \rVert_{p}:=\left( \sum_{n=1}^{\infty}|a_{n}|^{p} \right)^{1/p}$. 
