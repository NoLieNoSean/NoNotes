...
> > 
> > It remains to show $\overline{f}$ is uniformly continuous. Let $\epsilon> 0$. Let $\delta$ be chosen by the uniform continuity of $f$. We will show that the same $\delta$ works for $\overline{f}$. Let $x, y\in \tilde{X}$. Let $\{ x_{n} \}, \{ y_{n} \}\subseteq {X}$ such that $\{ x_{n} \}\to x$ and $\{ y_{n} \}\to y$. Note that $d(x_{n}, y_{n})\to d(x, y)$. If $d(x, y)<\delta$, then there exists $N$ such that for all $n\geq N$, $d(x_{n}, y_{n})<\delta$, so $d(f(x_{n}), f(y_{n}))<\epsilon$. Now, 
> > $$
> > \begin{align}
