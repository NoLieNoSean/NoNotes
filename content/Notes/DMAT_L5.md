...
> 
> So, by [[DMAT_L3#Zorn's lemma|Zorn's lemma]] , the partial order $P$ has a maximal element $\overline{G}=(V, \overline{E})$. Consider the binary relation $x, y\in V$, $x\sim y$ iff $\{ x, y \}\not\in \overline{E}$. Evidently, $\sim$ is reflexive and symmetric. We will show that it is transitive.
> 
