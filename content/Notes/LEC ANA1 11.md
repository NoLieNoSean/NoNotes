...
>- The [[LEC ANA1 10#Definition of limit of a function in metric spaces|definition]] of limit of a function from yesterday. Recall why we required $p$ to be a limit point.  
>
>
...
See @rudinPrinciplesMathematicalAnalysis1976 chapter 4.
...
How does this definition interact with limit points? Suppose $f$ is continuous at $p$, and $p$ is a limit point of $E$. Given the latter, 4.5 is a stricter statement than 4.1 - It says that the limit at $p$ exists, and that it equals $f(p)$ (point 3 above). This is essentially the familiar high school definition: $f$ is continuous at $p$ iff $\lim_{ x \to p }f(x)=f(p)$. 

If $p$ is not a limit point, then there must exist $\delta>0$ such that $B_{\delta}(p)\cap E$ contains no point other than $p$ (note that $B_{\delta}(p)$ always contains $p$). However, since the definition goes $d_{X}(x, p)<\delta$ and not $0<d_{X}(x, p)<\delta$, $x=p$ passes, and the "such that" clause becomes non-vacuously true. Note that even if we had the tighter version, the "such that" clause would still be vacuously true for this $\delta$. Thus, $f$ is continuous at $p$ (in fact, ANY function with domain $E$ is continuous at $p$). Also, it follows that $f$ is continuous at all isolated points in its domain.

[[#^801672]] can be restated like so:
...
A further simplification can be made. The previous definitions defined $f$ on a subset $E$ of a metric space $X$. However, (unlike the definition of limit) the definition of continuity does not involve the points of $X$ not in $E$ in any manner. This, combined with the knowledge that any subset of a metric space is a metric space, allows us to directly define $f$ on some metric space $E$. 
...
The result from the previous section has been used to simplify the theorem from how it appears in Rudin. 
### The sequence criterion

> [!Theorem]
> Let $X$ and $Y$ be metric spaces, $E\subset X$, $f:E\to Y$, $p$ is a limit point of $E$.
> Then, $\lim_{ x \to p }f(x)=q$ iff for every sequence $(p_{n})$ in $E\setminus \{ p \}$ such that  $\lim_{ n \to \infty }p_{n}=p$, we have $\lim_{ n \to \infty }f(p_{n})=q$.
> 
> > [!Proof]-
> > $(\implies)$ Take a given sequence $(p_{n})\to p$. Let $\epsilon>0$. Want to find $N$ such that for all $n>N$, $d(f(p_{n}), q)<\epsilon$. Since $\lim_{ x \to p }f(x)=q$, there must exist $\delta$ such that $0<d(p_{n},p)<\delta\implies d(f(p_{n}),q)<\epsilon$. Take $N$ such that for all $n>N$, $0<d(p_{n}, p)<\delta$, which ensures $d(f(p_{n}), q)<\epsilon$. 
> > 
> > $(\impliedby)$ Proof by contradiction. Suppose $\lim_{ x \to p }f(x)=q$ is false (see how this is different from having $\ne$ here). Then, there exists a "bad" $\epsilon$ such that for all $\delta>0$, there exists some $x$ such that $0<d(x, p)<\delta$ and $d(f(x), q)\geq \epsilon$. 
...
Follows from [[#^78c16d]] and [[Sequences and convergence#Uniqueness of a limit|the uniqueness of the limit of a sequence]].
...
> 
> > [!Proof]-
> > For every $(p_{n})\to p$ in $E\setminus \{ p \}$, we have $(f(p_{n}))\to a$ and $(g(p_{n}))\to b$, courtesy [[#^78c16d]] ($\implies$). Then, the [[Sequences and convergence#Algebraic limit theorem|algebra of limits of sequences]] tells us that $(f(p_{n})+g(p_{n}))\to a+b$. This implies $\lim_{ x \to p }(f(x)+g(x))=a+b$, also because of [[#^80ad34]] $(\impliedby)$.  
> > 
...
Briefly discussed Rudin 4.9 to 4.12. Noted the use of slot-wise convergence in proving 4.10.
