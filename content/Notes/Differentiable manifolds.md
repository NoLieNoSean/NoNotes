...
tags:
---
> [!Definition] @schultensIntroduction3manifolds2014 1.2.2
...
> 
> > [!Proof]-
> > Let $M, N$ be smooth manifolds. We've already [[Topological manifolds#^5ea3e2|shown]] that $M\times N$ is a manifold. Showing that $M\times N$ is in fact smooth entails showing that for every pair of charts $(M_{\alpha}\times N_{\alpha}, \phi_{\alpha}\times \psi_{\alpha})$, $(M_{\beta}\times N_{\beta}, \phi_{\beta}\times \psi_{\beta})$, the transition map $(\phi_{\alpha}\times \psi_{\alpha})\circ(\phi_{\beta}\times \psi_{\beta})^{-1}=(\phi_{\alpha}\times \psi_{\alpha})\circ(\phi_{\beta}^{-1}\times \psi_{\beta}^{-1})$ is smooth. Since $\phi_{\alpha}\circ \phi_{\beta}^{-1}$ and $\psi_{\alpha}\circ \psi_{\beta}^{-1}$ are given to be smooth and the product of smooth maps is smooth, it suffices to observe that the following diagram commutes, i.e., $(\phi_{\alpha}\times \psi_{\alpha})\circ(\phi_{\beta}^{-1}\times \psi_{\beta}^{-1})=(\phi_{\alpha}\circ\phi_{\beta}^{-1})\times( \psi_{\alpha}\circ\psi_{\beta}^{-1})$:
> > 
> > ```latex
> > % latex-id: 90d5-ed85-11da-4581-9d22
> > \begin{document}
> > % https://q.uiver.app/#q= 
...
> [!Question]
> [@leeIntroductionTopologicalManifolds2000 p.107] defines orientability in TRIANG; we've defined orientability in DIFF above. #q How does one define orientability in TOP? This seems to require singular homology. 
