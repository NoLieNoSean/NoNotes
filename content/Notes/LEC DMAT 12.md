...
Then, the [[LEC DMAT 6#Counting functions|number of surjections]] is equal to
$$
\begin{align}
...
Let $X$ be the set of all permutations on $[n]$. Let $A_{i}\subset X$ be the set of all permutations which map $i$ to $i$. Then, the number of [[LEC DMAT 8#Derangements|derangements]] is equal to
$$
\begin{align} \\
\left| X\setminus \bigcup_{i=1}^{n} A_{i} \right|  & =\sum_{I\subseteq[n]}(-1)^{|I|}|A_{I}| \\
...
\begin{align}
 ...
 & 2.~f(I)=\sum_{J\supseteq I} (-1)^{|J\setminus I|}g(J) &  \forall I\in \mathcal{P}(X). 
\end{align}
$$
