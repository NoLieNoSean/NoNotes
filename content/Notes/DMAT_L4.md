...
> Let $B=\bigcup_{i\in I}B_{i}$ and $\mathbf{D}=\bigcup_{i\in I}\mathbf{D}_{i}$. It is easy to see that $\mathbf{D}$ is a disjoint covering for $B$. $(B, \mathbf{D})\in P$, so [[DMAT_L3#Zorn's lemma|Zorn's lemma]] is applicable. Let $(\hat{B}, \mathbf{\hat{D}})$ be the maximal element. 
> 
...
> **Proof of 2**
> 
> Let If $|A|\leq |B|$, we have $|B|\leq |A\cup B|\leq |B\cup B|$, since trivial injections exist from sets on the left of the inequalities to sets on the right. It follows that $|B|\leq |A|+|B|\leq |B|+|B|=|B|$. From the [[DMAT_L2#Schröder–Bernstein theorem|Schröder–Bernstein theorem]], it follows that $|A|+ |B|=|B|$. 
> 
> **Proof of 3**
> 
