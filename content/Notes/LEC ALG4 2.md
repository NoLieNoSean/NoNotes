...
> [!Remark]
> Since every vector space has a basis ([[Hamel bases#^9b8388]]), it follows that every module over a field is free. Here's another proof: Let $M$ be a $k$-module. Let $A$ be a minimal generating set of $M$. We have a surjection $\varphi:k^{\oplus A}\twoheadrightarrow M$. Suppose $\ker\varphi$ is nontrivial, i.e, there exists a relation $\sum_{i=1}^{n} c_{i}a_{i}=0$ in $M$, where $a_{1}, \dots, a_{n}\in A$. Since $k$ is a field, we can write
> $$
> \begin{align}
