...
> [!Lemma] Zorn
> Let $(S, \leq)$ be any partially ordered set such that every chain has an upper bound. Then, $S$ has a maximal element.
...
> $$
> Clearly, $f'$ is well defined, and is injective. Thus, $(C', f')\in S$ is an upper bound for $K$. By Zorn's lemma, $S$ has a maximal element $(M, g)$, $M\subset A$, $g:M\to B$, injective.
> 
...
Now, several non-trivial conclusions can be drawn using [[LEC DMAT 2#Cardinal arithmetic|cardinal arithmetic]].
