...
time: 11:50
---
Recall what an [[ALG1_L21#Inner Product Spaces|inner product space]] is. Now, define $C[a, b]\equiv \{ f:[a, b]\to \mathbb{C}:f\text{ is continuous} \}$. Define
$$
...
$$
> [!Theorem]
> $C[a, b]=(C[a, b], \lVert \cdot \rVert_{\infty})$ is a [[CAL1_L1#Preliminaries|complete normed linear space]]. 

> **Proof**
> We have to show that every Cauchy sequence in $C[a, b]$ converges (in $C[a, b]$, of course). Suppose $(f_{n})$ is a Cauchy sequence in $C[a, b]$. Then, given $\epsilon>0$, $\exists N\in \mathbb{N}$ such that $n_{1}, n_{2}\geq N\implies \lVert f_{n_{1}}-f_{n_{2}} \rVert_{\infty}<\epsilon \implies \sup_{x\in[a, b]}|f_{n_{1}}(x)-f_{n_{2}}(x)|<\epsilon$. So, $(f_{n})$ satisfies the [[ANA1_L33#The Cauchy Criterion|Cauchy criterion]] for [[ANA1_L33#Uniform convergence|uniform convergence]]. Thus, $(f_{n})\to f$, where $f$ is continuous.
