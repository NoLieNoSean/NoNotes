...
> **Proof**
> We know that $f+g$ and $f-g$ are integrable on $[a, b]$, from [[LEC ANA1 27#a|6.12a]]. We can [[LEC ANA1 27#Compositions of integrable functions with continuous functions|compose]] these with $x^{2}$ to obtain integrable functions $(f+g)^{2}$ and $(f-g)^{2}$. It follows that $(f+g)^{2}+(f-g)^{2}=4fg$ is integrable. ❏
...
> **Proof**
> Let $H(x)=F(x)G(x)$. Then, $h\equiv H'=fG+Fg$. Note that since $f, g\in\mathscr{R}$ and $F, G\in\mathscr{R}$(since [[LEC ANA1 23#Differentiability implies continuity|differentiability implies continuity]] and [[LEC ANA1 27#Continuous functions|continuous functions are integrable]]), $h\in\mathscr{R}$. From the fundamental theorem, we have
> $$
