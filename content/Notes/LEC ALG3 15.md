...
> > [!Proof]-
> > 
> > Choose $\alpha\in L$. $\alpha$ is algebraic over $K$. Let $\text{Irr}(\alpha; K)=\sum_{i=0}^{n}a_{n}x^{n}$. The key observation is that $\alpha$ is algebraic over the finitely generated extension $F(a_{0}, \dots, a_{n})$ of $F$. Also, the extension $F(a_{0}, \dots, a_{n}, \alpha)/F(a_{0}, \dots, a_{n})$ is finite. By [[LEC ALG3 16#^ead96e]], the extension $F(a_{0}, \dots, a_{n}, \alpha)/F$ is finite, and hence algebraic by [[LEC ALG3 16#^bacf07]]. Thus, $\alpha$ is algebraic over $F$. 
> 
