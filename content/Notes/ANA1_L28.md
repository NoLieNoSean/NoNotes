...
> [!Recall]
> [[ANA1_L27#Which functions are integrable?|Integrability]] of bounded $f$ on $[a, b]$:
> 
...
> **Proof**
> For $f\in\mathscr R$ to be true, $f$ must be bounded. Suppose $|f(t)|\leq M$ in $[a, b]$. We will prove that $F$ is [[ANA1_L20#Uniform continuity|uniformly continuous]] (uniform continuity implies continuity). Let $a\leq x<y\leq b$. Let $\epsilon>0$. Then, if $y-x < \frac{\epsilon}{M}$, we have
> $$
...
> **Proof**
> Let $\epsilon>0$. Since $f\in\mathscr{R}$ on $[a, b]$, we can choose a partition $P$ of $[a, b]$ such that $U(P, f)-L(P, f)<\epsilon$. Because of the [[ANA1_L24#Mean value theorem|mean value theorem]], we can choose $t_{i}\in[x_{i-1}, x_{i}]$ such that
> $$
...
> **Proof**
> Let $H(x)=F(x)G(x)$. Then, $h\equiv H'=fG+Fg$. Note that since $f, g\in\mathscr{R}$ and $F, G\in\mathscr{R}$(since [[ANA1_L23#Differentiability implies continuity|differentiability implies continuity]] and [[ANA1_L27#Continuous functions|continuous functions are integrable]]), $h\in\mathscr{R}$. From the fundamental theorem, we have
> $$
