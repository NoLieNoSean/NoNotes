...
>[!Definition]
>For a metric space $X$, a *separation* of $X$ is a partition of $X=A\sqcup B$ into two proper subsets $A$ and $B=X\setminus A$ such that $A$ and $B$ are both [[ANA1_L13#Open sets|open]] in $X$. 
...
>[!Theorem]
>If $X$ is a connected metric space and $f:X\to Y$ is [[ANA1_L13#Halfway through to a new definition of continuity|continuous]], $f(X)$ is connected.
...
>**Proof**
>Let $C\subset X$ be connected. Let $A\sqcup B=X$ be a separation of $X$. FTSOC, assume $C\cap A\ne \emptyset$ and $C\cap B\ne \emptyset$. [[ANA1_L15#Subspace topology|Since]] $A$ and $B$ are open in $X$, $C\cap A$ and $C\cap B$ are open in $C$. We can write $C=(C\cap A)\sqcup(C\cap B)$, i.e, $C$ is the disjoint union of two nonempty sets open in $C$. But this means $C$ is disconnected! $\Rightarrow\Leftarrow$ ❏

>[!Theorem] Theorem 2
>If a family of connected subsets of $X$ have a point in common, then their union is connected.

>**Proof**
>Let $\{ C_{\alpha} \}$ be a family of connected subsets of $X$, and let $p\in \bigcap_{\alpha}C_{\alpha}$. FTSOC, assume $C\equiv \bigcup_{\alpha}C_{\alpha}$ is disconnected, i.e, there exist non empty, disjoint $A$ and $B$ such that $A\sqcup B=C$ and $A$ and $B$ are open in $C$. WLOG, $p\in A$. Consider $q\in B$. There must exist $C_{\alpha}$ such that $p, q\in C_{\alpha}$. [[ANA1_L15#Subspace topology|Since]] $A$ and $B$ are open in $C$, it follows that $A\cap C_{\alpha}$ and $B\cap C_{\alpha}$ are open in $C_{\alpha}$. We can write $C_{\alpha}=(A\cap C_{\alpha})\sqcup (B\cap C_{\alpha})$, i.e, $C_{\alpha}$ is the the disjoint union of two nonempty sets open in $C_{\alpha}$. But this means $C_{\alpha}$ is disconnected! $\Rightarrow\Leftarrow$ ❏
...
The components are also open in $X$ if their number is finite, [[ANA1_L14#Open and closed sets under intersections and unions|since finite unions of closed sets are closed]].
