...
tags:
---
Definition of differentiability. Note that since all norms on a fdvsp are equivalent, the definition does not depend on the norm. If $F$ is differentiable, its directional derivatives exist and are equal to the total derivate evaluated at $v$. More generally suppose $F$ is differentiable at $a$, and $\gamma:(-\epsilon, \epsilon)\to U$ is differentiable with $\gamma(0)=a$ and $\gamma'(0)=v$. Then
$$
...
See @deySelectedTopicsMultivariable
