...
> $$
> Then $f(\mathbf{b})-f(\mathbf{a})=g(1)-g(0)$. Note that $g^{(1)}, \dots, g^{(m-1)}$ are continuous on $(-\delta, 1+\delta)$, and $g^{m}$ is defined on $(-\delta, 1+\delta)$. By applying the [[ANA1_L25#Taylor's theorem|one dimensional Taylor formula]] to $g$, we get
> $$
> g(1)-g(0)=\sum_{k=1}^{m-1} \frac{1}{k!}g^{(k)}(0)+\frac{1}{m!}g^{(m)}(\theta),\quad \quad(*)
> $$
> where $0<\theta< 1$. 
> 
> Now $g$ is a composite function given by $g(t)=f(\mathbf{p}(t))$, where $\mathbf{p}(t)=\mathbf{a}+t(\mathbf{b}-\mathbf{a})$. From the [[CAL1_L17#The chain rule|chain rule]], we have
> $$
> \begin{align}
