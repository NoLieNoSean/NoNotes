...
\end{array}
$$
Now, from the [[ALG1_L11#Rank nullity theorem for general linear maps over fdvsps|rank nullity theorem]], $\dim \mathrm{Im}\ \psi=\dim V$. Note that $\dim V= \dim\hat{V}=\dim \hat{\hat{V}}$. Hence, $\psi$ is surjective. Therefore, $\psi$ is a bijective linear map, i.e, an isomorphism.
$$
