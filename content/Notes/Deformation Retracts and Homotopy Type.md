...
However, there exist spaces that are not deformation retracts of each other, but still have isomorphic fundamental groups (like the figure eight space and the theta space, for instance). This suggests the possibility that there might be a more general way of showing two spaces have isomorphic fundamental groups. 
...
For example, if $A$ is a deformation retract of $X$, then $A$ has the same homotopy type as $X$: if we let $j:A\hookrightarrow X$ be the inclusion and let $r:X\to A$ be the retraction mapping, then the composite $r\circ j$ *equals* the identity map of $A$, and the composite $j\circ r$ is by hypothesis homotopic to $\mathrm{id}_{X}$. 
...
> Let $f:X\to Y$ be continuous; let $f(x_{0})=y_{0}$. If $f$ is a homotopy equivalence, then
> $$
> f_{*}:\pi_{1}(X, x_{0})\to \pi_{1}(Y, y_{0})
> $$
> is an isomorphism.

The theta space and the figure eight space are both deformation retracts of the doubly punctured plane. Therefore, they are homotopy equivalent to each other. 

In fact, two spaces $X$ and $Y$ have the same homotopy type iff they are homeomorphic to deformation retracts of a single space: the mapping cylinder of a homotopy equivalence $f:X\to Y$ deformation retracts onto homeomorphic images of $X$ and $Y$. 
...
> Let $f:X\to Y$ be continuous. Then, $Y$ is a deformation retract of $M_{f}$. If $f$ is a homotopy equivalence, then $X\times \{ 0 \}\subseteq M_{f}$ is a strong deformation retract of $M_{f}$. 
