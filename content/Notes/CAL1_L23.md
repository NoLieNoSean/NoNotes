...
> 
> > [!Proof]-
> > Since $f^{(n)}(c)\ne 0$ and $f^{(n)}$ is continuous at $c$, there exists an interval $B(c)$ such that for every $x$ in $B(c)$, $f^{(n)}(x)$ will have the same sign as $f^{(n)}(c)$. Now by [[ANA1_L25#Taylor's theorem|Taylor's formula]] for functions from $\mathbb{R}$ to $\mathbb{R}$, for every $x\in B(c)$,
> > $$
> > \begin{align}
