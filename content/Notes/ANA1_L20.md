...
>[!Recall]
>Definition of [[ANA1_L17#Absolute and relative definitions of open cover compactness|open cover compactness]].
...
Let $f:X\to \mathbb{R}$ be a continuous function. Let $X$ be compact. The [[ANA1_L16#Extreme value theorem|extreme value theorem]] tells us that $f(x)$ attains a maximum and minimum value, and the previous theorem tells us that $f$ is uniformly continuous.

We will now show that these theorems do not hold when $X$ is not compact. 

We know from the [[ANA1_L16#Heine Borel Theorem|Heine Borel theorem]] that in $\mathbb{R}$, compact $\iff$ closed and bounded.
...
>**Proof of 1**
>Note that in all the following examples $f$ is continuous since [[ANA1_L15#Restrictions of continuous functions are continuous|restrictions of continuous functions are continuous]].
