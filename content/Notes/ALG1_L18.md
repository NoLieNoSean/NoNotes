...
Recall definitions of [[AlG1_L17|eigenvalues, eigenvectors, and eigenspaces]].
...
> 
> If use [[ALG1_L15#Cofactor expansions|cofactor expansion]] along the first row to compute the determinant, observe that only the first term in the expansion will have a $x^{n}$ and $x^{n-1}$ term. Thus, we only need to bother with the first term.
> $$
> \begin{align}
...
> $$
> since the determinant of a triangular matrix is the [[ALG1_L15#Computing the determinant|product of its diagonal entries]]. We can see that the diagonal entries are the eigenvalues of $A$.
