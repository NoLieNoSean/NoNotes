...
> [!Theorem]
> Suppose $\mathbf{f}$ maps a convex open set $E\subseteq \mathbb{R}^{n}$ to $\mathbb{R}^{m}$, $\mathbf{f}$ is differentiable in $E$, and there is a real number $M$ such that for every $\mathbf{x}\in E$, $\lVert \mathbf{f}'(\mathbf{x}) \rVert \leq M$. Then
> $$
