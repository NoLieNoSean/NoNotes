...
>[!Definition] Rudin 5.1
>Let $f: [a, b]\to \mathbb{R}$. For any $x\in[a, b]$, define the quantity
>$$
>f'(x)\equiv\lim_{ t \to x } \frac{{f(t)-f(x)}}{t-x}
>$$
>provided the [[LEC ANA1 10#Definition of limit of a function in R|limit exists]], in which case we say $f$ is *differentiable* at $x$.
...
> >$$
> >From the [[LEC ANA1 11#Algebra of limits of functions|algebra of limits]], this is equal to
> >$$
...
> 3. $\left( \frac{f}{g} \right)'(x)=\frac{{f'(x)g(x)-f(x)g'(x)}}{g^{2}(x)}$, if $g(x)\ne 0$.
> 
> >[!Proof]-
> >$(1)$ Follows from the [[LEC ANA1 11#Algebra of limits of functions|algebra of limits]]. 
> >
> >$(2)$ $f(t)g(t)-f(x)g(x)=(f(t)-f(x))g(t)+(g(t)-g(x))f(x)$. 
> >
> >
> >$(3)$ $\Large\frac{f(t)}{g(t)}-\frac{f(x)}{g(x)}=\frac{{(f(t)-f(x))g(x)-(g(t)-g(x))f(x)}}{g(x)g(t)}$.
> 
