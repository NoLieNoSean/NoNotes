...
Now, let the sign of a permutation be defined as the number of "disorders" in the permutation, as done [[ALG1_L16#Another formula for determinants|here]]. Note with this definition, the sign of a permutation is a well defined quantity. It can easily be verified that if a cycle $\sigma$ has length $k$,  $\text{sgn}(\sigma)=(-1)^{k-1}$. Thus, if $\tau = \sigma_{1}\sigma_{2}\dots\sigma _k$, $\text{sgn}(\tau)=\text{sgn}(\sigma_{1})\text{sgn}(\sigma_{2})\dots\text{sgn}(\sigma_{k})$. 
