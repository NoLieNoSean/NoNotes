...
> **Proof**
> FTSOC, assume $|P|=p^{n}$ and $|Z(P)|=p^{n-1}$. Then, $|P/Z(P)|=p$, so $P/Z(P)$ is cyclic. [[LEC ALG2 8#Some theorems|Thus]], $P$ is abelian. But this implies $|Z(P)|=p^{n}$, a contradiction.
...
> Lagrange's theorem forces $\langle x \rangle\cap \langle y \rangle=\{ 1 \}$. Thus, $|\langle x \rangle\langle y \rangle|=p^{2}$, so $\langle x \rangle\langle y \rangle=P$. These facts along with $\langle x \rangle\triangleleft P$ and $\langle y \rangle\triangleleft P$ [[LEC ALG2 6#Direct products|allow us]] to write $P=\langle x \rangle\times \langle y \rangle$. Thus, $P\cong \mathbb{Z}_{p}\times \mathbb{Z}_{p}$. 
